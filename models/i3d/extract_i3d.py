...
class ExtractI3D(BaseExtractor):
    ...
    def __init__(self, args) -> None:
        ...
        super().__init__(
            feature_type=args.feature_type, ## 'i3d'
        ...
        self.flow_type = args.flow_type ## 'raft'
        self.i3d_classes_num = 400
        self.min_side_size = 256
        self.central_crop_size = 224
        self.extraction_fps = args.extraction_fps  ## 15 fps
        ## step_size = 64 -> 32 로 수정
        self.step_size = 32 if args.step_size is None else args.step_size
        ## stack_size = 64
        self.stack_size = 32 if args.stack_size is None else args.stack_size
        self.resize_transforms = torchvision.transforms.Compose([
            torchvision.transforms.ToPILImage(),
            ResizeImproved(self.min_side_size), ## 더 짧은 변이 256이 되도록 resize
            PILToTensor(),
            ToFloat(),
        ])
        self.i3d_transforms = {
            'rgb': torchvision.transforms.Compose([
                TensorCenterCrop(self.central_crop_size),    ## 중심을 기준으로 (224, 224) 
                ScaleTo1_1(),
                PermuteAndUnsqueeze()
            ]),
    ...
    def extract(self, video_path: str) -> Dict[str, np.ndarray]:
        ...
        while cap.isOpened():
            ...
            if frame_exists:
                ...
                rgb = self.resize_transforms(rgb)   ## 짧은 변이 256이 되도록 비율 유지 resize
                rgb = rgb.unsqueeze(0)
    ...
    def run_on_a_stack(self, rgb_stack, stack_counter, padder=None) -> Dict[str, torch.Tensor]:
        ...
        for stream in self.streams:
            ...
            stream_slice = self.i3d_transforms[stream](stream_slice)  ## 중심을 기준으로 (224, 224)
